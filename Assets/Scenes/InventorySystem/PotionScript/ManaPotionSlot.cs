...
public class ManaPotionSlot : MonoBehaviour, IPointerClickHandler
{
    ...
    public Slider manaThresholdSlider;
    void Update()
    {
        if (playerManager.GetCurrentMana() <= playerProperty.maxMana * manaThresholdSlider.value && manaThresholdSlider.value != 1)
        {
            foreach (InventorySlot slot in inventory.inventorySlots)
            {
                if (slot.item != null && slot.item.itemName == "Mana Potion" && slot.stack > 0)
                {
                    slot.UseItem();
