...
{
    [SerializeField] private GameObject DarkBoltEffectPrefab;
    ...
    public void DarkBolt_OnSkillStart()
    {
        base.OnSkillStart();
    }
    ...
    public void DarkBolt_OnEffectSpawn()
    {
        
        base.OnEffectSpawn();
        Vector3 spawnPosition = transform.position + transform.rotation * hitboxOffset;
        spawnedEffect = Instantiate(DarkBoltEffectPrefab, spawnPosition, DarkBoltEffectPrefab.transform.rotation);
    
        // ปรับขนาดตามธาตุ
        spawnedEffect.transform.localScale = CalculateElementalScale(effectScale);
        // **Delay 0.5 วินาทีก่อนเปิด Hitbox**
        Invoke(nameof(DarkBolt_OnHitboxActivate), hitboxSpawnDelay);
        Destroy(spawnedEffect, 6f);
    }
    public void DarkBolt_OnHitboxActivate()
    {
    ...
    public  void DarkBolt_OnSkillEnd()
    {
        base.OnSkillEnd();
    }
}
