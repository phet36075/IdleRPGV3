...
public class FrozenKnife : BaseSkill
{
    [SerializeField] private GameObject FrozenKnifeEffectPrefab;
    ...
    public void FrozenKnife_OnSkillStart()
    {
        base.OnSkillStart();
    }
    ...
    public void FrozenKnife_OnEffectSpawn()
    {
        
        base.OnEffectSpawn();
        Vector3 spawnPosition = transform.position + transform.rotation * hitboxOffset;
        spawnedEffect = Instantiate(FrozenKnifeEffectPrefab, spawnPosition, FrozenKnifeEffectPrefab.transform.rotation);
    
        // ปรับขนาดตามธาตุ
        spawnedEffect.transform.localScale = CalculateElementalScale(effectScale);
        // **Delay 0.5 วินาทีก่อนเปิด Hitbox**
        Invoke(nameof(FrozenKnife_OnHitboxActivate), hitboxSpawnDelay);
        Destroy(spawnedEffect, 5f);
    }
    public void FrozenKnife_OnHitboxActivate()
    {
    ...
    public void FrozenKnife_OnSingleHitboxActivate()
    {
        SkillHitbox hitbox = spawnedEffect.GetComponent<SkillHitbox>();
        if (hitbox != null)
        {
    ...
    public  void FrozenKnife_OnSkillEnd()
    {
        base.OnSkillEnd();
    }
}
