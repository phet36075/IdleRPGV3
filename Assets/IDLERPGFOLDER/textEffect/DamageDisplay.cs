...
{
    public GameObject damageTextPrefab, damageTextPrefabCritical,damageTextPrefabWinElement,healingTextPrefab,restoreManaTextPrefab;
    ...
    public void DisplayHealing(float amount)
    {
        ...
        // สร้าง instance ของ prefab ที่ตำแหน่งที่คำนวณไว้ โดยไม่กำหนด parent
        GameObject damageTextInstance = Instantiate(healingTextPrefab, spawnPosition, Quaternion.identity);

        // ตั้งค่าข้อความให้แสดงค่าความเสียหาย
        damageTextInstance.GetComponentInChildren<TextMeshPro>().SetText(amount.ToString("0"));
    }

    public void DisplayRestoreMana(float amount)
    {
        // คำนวณตำแหน่งที่จะแสดงตัวหนังสือ
        Vector3 spawnPosition = textLocation.position;
        ...
        // สร้าง instance ของ prefab ที่ตำแหน่งที่คำนวณไว้ โดยไม่กำหนด parent
        GameObject damageTextInstance = Instantiate(restoreManaTextPrefab, spawnPosition, Quaternion.identity);
